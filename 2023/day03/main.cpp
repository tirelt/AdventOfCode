...
using ptr_map=shared_ptr<map<pair<unsigned int,unsigned int>,unsigned int>>;
...
bool check_arround(ptr_matrix& matrix_ptr, int i, int j, int k,int number_lines,int length){
...
int main(){
    ...
    ptr_map map_ptr = make_shared<map<pair<unsigned int,unsigned int>,unsigned int>>();
    unsigned int sum = 0,j,k,n=0,length=(*matrix_ptr)[0].size(),number_lines=matrix_ptr->size();
    for(unsigned int i=0;i<number_lines;++i){
        ...
        while(j<length){
            ...
            if(isdigit(c)){
                ...
                if(check_arround(matrix_ptr, i, j, k-1,number_lines,length)){
                    sum += n;
                }
                j=k+1;
            } else{
